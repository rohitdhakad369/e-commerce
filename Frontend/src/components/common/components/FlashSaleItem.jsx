  ...
  return (
    <div className="relative mx-2 ">
      ...
      >
        ...
        {item.discount && (
          <div className="absolute top-0 left-0 bg-zinc-600 text-white py-1 px-3 m-2 rounded">
            -{item.discount}%
          </div>
        )}

        {item.state && (
          <div className="absolute top-0 left-0 bg-lime-700 text-white py-1 px-3 m-2 rounded">
            {i18n.t("new")}
          </div>
        )}
      ...
      <div className="flex md:items-start items-center flex-col ">
        <h3 className="text-sm font-base mt-4">{item.title}</h3>
        <p className=" text-gray-950  text-sm  font-bold line-clamp-2">
        ₹{item.price}
          {item.discount && (
        ...
        <span>
          <div className="flex mt-2 text-gray-500 text-sm font-semibold gap-2 items-center ">
            {renderStars()} <span className=" text-xs">{item.rates}</span>
